...
namespace RevitWebAppSync.UI.Copilot.Model
{
    ...
    public sealed class SlashTool
    {
        ...
        /// <summary>The bina-ai P1 command id sent to the backend as
        /// `command_id` (authoritative dispatch). Defaults to `Id` when the
        /// addin-local id already matches the backend; set explicitly where
        /// they differ. `null`/empty → this tool has no backend command yet
        /// (menu shows it, but a pick falls back to a plain NL prompt).</summary>
        public string CommandId;

        /// <summary>The backend id to dispatch — CommandId if set, else Id.</summary>
    ...
    public static class ToolCatalog
    {
        ...
        // addin-local Id → bina-ai P1 command id (from app/commands/*.md).
        // Only the ids that differ are listed; the rest already match.
