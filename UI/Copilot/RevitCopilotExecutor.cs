...
namespace RevitWebAppSync.UI.Copilot
{
    ...
    public class RevitCopilotExecutor : ICopilotExecutor
    {
        ...
        private static string BuildOpenView(string viewName)
        {
            ...
            sb.AppendLine("    // Type-based fallback. Build a ranked candidate list and try each —");
            sb.AppendLine("    // OfClass(typeof(View3D)) returns BOTH user views AND Revit-internal");
            sb.AppendLine("    // ones like 'Project View' (synthetic, non-switchable). The only");
            sb.AppendLine("    // reliable way to tell them apart is to ask Revit to activate each;");
            sb.AppendLine("    // RequestViewChange throws ArgumentException on internal/render views.");
            sb.AppendLine("    var __q = (__name ?? \"\").ToLowerInvariant();");
            sb.AppendLine("    List<View> __cands = new List<View>();");
            sb.AppendLine("    if (__q.Contains(\"3d\")) {");
            sb.AppendLine("        __cands = new FilteredElementCollector(doc).OfClass(typeof(View3D)).Cast<View3D>()");
            sb.AppendLine("            .Where(x => !x.IsTemplate)");
            sb.AppendLine("            .OrderByDescending(x => x.Name == \"{3D}\")    // default 3D wins");
            sb.AppendLine("            .ThenByDescending(x => !x.IsPerspective)       // then orthographic");
            sb.AppendLine("            .Cast<View>().ToList();");
            sb.AppendLine("    } else {");
            ...
            sb.AppendLine("        if (__t != ViewType.Undefined) {");
            sb.AppendLine("            __cands = new FilteredElementCollector(doc).OfClass(typeof(View)).Cast<View>()");
            sb.AppendLine("                .Where(x => !x.IsTemplate && x.ViewType == __t).ToList();");
            sb.AppendLine("        }");
            sb.AppendLine("    }");
            sb.AppendLine("    foreach (var __cand in __cands) {");
            sb.AppendLine("        try { uidoc.RequestViewChange(__cand); __v = __cand; break; }");
            sb.AppendLine("        catch (Autodesk.Revit.Exceptions.ArgumentException) { /* internal — try next */ }");
            sb.AppendLine("    }");
            sb.AppendLine("    if (__v != null) {");
            sb.AppendLine("        SetResult(new { kind = \"plain\", headline = \"Opened \" + __v.Name, sub = \"Switched the active view.\" });");
            sb.AppendLine("        return null;");
            sb.AppendLine("    } else if (__cands.Count > 0) {");
            sb.AppendLine("        string __kind = __q.Contains(\"3d\") ? \"3D view\" : \"view\";");
            sb.AppendLine("        SetResult(new { kind = \"plain\", headline = \"No switchable \" + __kind, sub = \"All \" + __cands.Count + \" candidate(s) in this project are internal or non-switchable. Create a regular \" + __kind + \" (View > 3D View / Plan / etc.) and try again.\" });");
            sb.AppendLine("        return null;");
            sb.AppendLine("    }");
            sb.AppendLine("    // No candidates at all — fall through to 'View not found' below.");
            sb.AppendLine("}");
            sb.AppendLine("if (__v != null) {");
            sb.AppendLine("    try {");
